CREATE PROCEDURE [dbo].[CrearSolicitudesDeRecibo]
    @id_archivo BIGINT
AS
BEGIN TRY
    --SET NOCOUNT ON;
    ...
    BEGIN
        ...
        SELECT
            ...
            ,b.ciudad_codigo_alterno AS ciudad_remitente_codigo_alterno
            ,b.direccion AS direccion_remitente
            ,CAST(NULL AS BIGINT) AS id_direccion_remitente
            ,b.id_punto AS id_punto_remitente
            ,b.punto_codigo_alterno AS punto_remitente_codigo_alterno
            ,b.punto_nombre AS punto_remitente_nombre
            ,CAST(0 AS BIT) AS requiere_cita_remitente
            ,a.fecha_maxima_solicitada AS fecha_cita_remitente
            ,CAST(NULL AS TIME(0)) AS hora_cita_minima_remitente
            ,CAST(NULL AS TIME(0)) AS hora_cita_maxima_remitente

            ,c.id_ciudad AS id_ciudad_destinatario
            ,d.codigo AS ciudad_destinatario_codigo_alterno
            ,c.direccion AS direccion_destinatario
            ,c.id_direccion AS id_direccion_destinatario
            ,CAST(NULL AS BIGINT) AS id_punto_destinatario
            ,c.codigo AS punto_destinatario_codigo_alterno
            ,c.nombre AS punto_destinatario_nombre
            ,CAST(0 AS BIT) AS requiere_cita_destinatario
            ,a.fecha_minima_solicitada AS fecha_cita_destinatario
