CREATE PROCEDURE [dbo].[MergeMaestroTiposRemesaFromProduction2Test] AS
BEGIN
    BEGIN TRY
        BEGIN TRANSACTION

        BEGIN
            MERGE [$(eConnect)].dbo.tipos_remesa AS t
            USING [$(SERVER_MASTER_DATA)].[$(eConnectMasterData)].dbo.tipos_remesa AS s ON 
                s.id_tipo_remesa = t.id_tipo_remesa
